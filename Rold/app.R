#
# Shiny app for the size-based plankton simulator
#
library(shiny)
require(latex2exp)
source("basetools.R")
...
ui <- fluidPage(
  ...
  p('THIS IS WORK IN PROGRESS. Version 0.5. November 2018. 
    Code: https://github.com/Kenhasteandersen/SizeBasedPlankton.')
  ,
      ...
      conditionalPanel(
        condition = "input.amplitudeL>0",
          sliderInput("t",
                      ...
                      value=120,
                      width="700px"),
          plotOutput("plotTimeline", width="700px", height="150px")
        ),
...
server <- function(input, output) {
  ...
  #
  # Plot light and diffusivity for the seasonal simulation:
  #
  output$plotTimeline <- renderPlot({
    defaultplot()
    t = seq(0,365)
    L = light(t, input$L, input$amplitudeL)
    defaultpanel(xlim=c(0,365), ylim=c(0,max(c(L, input$d))))

    lines(t, L, lwd=3, col="orange")
    lines(t, diffusion(t, input$d))
    vline(input$t)  
  })
