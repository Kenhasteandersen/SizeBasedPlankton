function [dudt, rates] = calcrates3(t,x,u, p, bReturnRates, gammaN, gammaDOC)
...
if (N<0)
    ...
    JN =   gammaN .* JmaxT/p.rhoCN .* ANmT*N ./ (JmaxT/p.rhoCN + ANmT*N); % Diffusive nutrient uptake
end
% in units of N/time
if (DOC<0)
    JDOC=zeros(10,1);
else
    JDOC = gammaDOC .* JmaxT .* ANmT*DOC ./ (JmaxT + ANmT*DOC); % Diffusive DOC uptake, units of C/time
end
...
JLreal = min(JL, max(0, JNtot*p.rhoCN - (JF+JDOC-JR)));
